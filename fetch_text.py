...
url = 'https://medium.com/search/posts?q=ethereum&count=500'
path = f'./{query}/'
doc_path = path + 'docs/'
...
def get_soup(link):
	soup=None
	cnt=5
	while soup is None and cnt>0:
		try:
			soup = BeautifulSoup(urlopen(Request(link, headers = header)).read(), 'html.parser')
		except:
			cnt-=1
			pass
	return soup
...
# while soups is None:
# 	try:
# 		soups = [get_soup(i) for i in links]
# 	except:
...
for count, link in enumerate(links):
	soup = get_soup(link)
	if soup != None:
		text = ' '.join([j.text for j in soup.find_all('p')])
		f=open(f'{path}/docs/{query}-{count}.html', 'w')
		f.write(text)
		f.close()
		print(f'File {count}:   {path}/docs/{query}-{count}.html')
# count=1
# texts = {links[indx]:' '.join([j.text for j in i.find_all('p')]) for indx, i in enumerate(soups)}
# for key, val in texts.items():
# 	f=open(f'{path}/docs/{query}-{count}.html', 'wb')
# 	f.write(val)
# 	f.close()
# 	count+=1
