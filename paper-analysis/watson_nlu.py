...
import os
metadocs_path = f'./meta-docs/'
try:
	os.mkdir(metadocs_path)
except FileExistsError:
	pass
# fnames = sorted(os.listdir(f'{query}/docs/'), key=lambda x: int(x.split('-')[1].split('.')[0]))
# f = open(f'./papers/{query}.txt', 'r')
import textract
txt = '.' in query and query.split('.')[1] == 'txt'
try:
	response = service.analyze(
		text=open(f'./papers/{query}.txt', 'r').read() if txt else textract.process(f'./papers/{query}').decode('utf-8'),
...
fjson = open(metadocs_path + f"{query}.json", 'w')
fjson.write(resp)
fjson.close()